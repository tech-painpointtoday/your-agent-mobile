...
import 'property_edit_event.dart';
import 'property_edit_state.dart';

/// BLoC for handling property edit flow
class PropertyEditBloc extends Bloc<PropertyEditEvent, PropertyEditState> {
  final PropertyApiService _propertyApiService;

  PropertyEditBloc({required PropertyApiService propertyApiService})
    : _propertyApiService = propertyApiService,
      super(PropertyEditInitial()) {
    on<PropertyEditLoad>(_onLoad);
    on<PropertyEditSubmitted>(_onSubmitted);
    on<PropertyEditReset>(_onReset);
  }

  Future<void> _onLoad(
    PropertyEditLoad event,
    Emitter<PropertyEditState> emit,
  ) async {
    ...
    try {
      ...
      emit(PropertyEditLoaded(property: property));
    } catch (e) {
      emit(PropertyEditFailure(e.toString()));
    }
  }

  Future<void> _onSubmitted(
    PropertyEditSubmitted event,
    Emitter<PropertyEditState> emit,
  ) async {
    ...
    // Emit submitting state if we have a property
    if (currentProperty != null) {
      emit(PropertyEditSubmitting(property: currentProperty));
    }
    ...
    try {
      ...
      if (name == null || name.trim().isEmpty) {
        // Reset to loaded state instead of emitting failure
        if (currentProperty != null) {
          emit(PropertyEditLoaded(property: currentProperty));
        }
      ...
      // Step 1: Update property using unified endpoint
      await _propertyApiService.saveProperty(role: roleName, data: payload);
      ...
      emit(PropertyEditSuccess(propertyId: event.propertyId));
    } catch (e) {
  ...
  void _onReset(PropertyEditReset event, Emitter<PropertyEditState> emit) {
