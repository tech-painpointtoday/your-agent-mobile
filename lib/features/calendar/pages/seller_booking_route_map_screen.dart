...
/// Seller route map screen: shows route from current location to booking property
/// with bottom sheet card (address, ETA, status, primary action).
class SellerBookingRouteMapScreen extends StatefulWidget {
  final int bookingId;

  const SellerBookingRouteMapScreen({super.key, required this.bookingId});

  @override
  State<SellerBookingRouteMapScreen> createState() =>
      _SellerBookingRouteMapScreenState();
}

class _SellerBookingRouteMapScreenState
    extends State<SellerBookingRouteMapScreen> {
  ...
  Future<void> _fetchRealRoute() async {
    ...
    // NOTE: For now we draw a simple straight polyline from origin to destination.
    // If you want full turn-by-turn routing, add `flutter_polyline_points`
    // and use the Google Directions API as in the youragent app.
    try {
      if (!mounted) return;
      setState(() {
        _polylines = {
                ...
                Row(
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        Text(
                          l10n.calendar_status_traveling,
                          style: GoogleFonts.anuphan(
...
class _BuyerTimingRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context);
    final display = arrivalTime ?? '--:--';
    return Text(
      '${l10n.booking_status_traveling_client} ($display)',
      style: GoogleFonts.anuphan(
